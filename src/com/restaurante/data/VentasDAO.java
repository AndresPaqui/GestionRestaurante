...
//revisar el array (corregido)
    public List<ItemVenta> getDetallesDeVenta(int idVenta) {
        List<ItemVenta> detalles = new ArrayList<>();
        String sql = """
                SELECT dv.cantidad, dv.subtotal,
                       p.id AS id_plato, p.nombre, p.precio_venta
        ...
        try (PreparedStatement ps = Conexion.conectar().prepareStatement(sql)) {
            ...
            System.err.println("[VentasDAO] getDetallesDeVenta: " + e.getMessage());
        }
        return detalles;
    }
        ...
        try (Statement st = Conexion.conectar().createStatement();
            ...
            while (rs.next()) {
                ...
                lista.add(new Venta(
                        idVenta,
                        fecha,
                        getDetallesDeVenta(idVenta),  // cambiado aqui
